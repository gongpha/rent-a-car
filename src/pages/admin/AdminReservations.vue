...
export interface Reservation {
	...
	status : 'RESERVED' | 'COMPLETED' | 'CANCELED' | 'IN_USE';
	note: string;
...
onMounted(() => {
	axios.get(apiURL + '/admin/reservations/0', {
		withCredentials: true
	}).then((response) => {
		if (response.data.reservations == 0) {
			// root
			isRoot.value = true
...
const onBranchChange = (event: Event) => {
	...
	axios.get(apiURL + '/admin/reservations/' + branchId, {
		withCredentials: true
	}).then((response) => {
		reservations.value = response.data.reservations
	}).catch((error) => {
...
<template>
	...
	<div class="admin-content">
		<h1 v-if="isRoot">การจองทั้งหมด</h1>
